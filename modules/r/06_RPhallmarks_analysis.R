...
### Load files###
# Load shap matrix
shap_relevant_stable_notscaled <- readRDS(file =  file.path(rds_folder,"shap_relevant_stable_notscaled_hallmarks.rds"))
# Load the filtered matrix with stable circuits and relevant KDTs
shap_relevant_stable <- read.delim(file = file.path(tables_folder,"shap_relevant_stable.tsv"))
# Load the drug_eff DF from Drugbank DB
drugbank_effects_tar<- readRDS(file.path(rds_folder, "drugbank_effects_tar.rds"))

# 6 validated KDTs
vali <- c("ALOX5", "ELOVL4", "GABRA1", "GRIN1", "SLC12A5", "GLRA2")
...
# get the nº of circuit per pathway to obtain normalized Freq of Hallmark per pathway
n_cir <- data.frame(table(sapply(str_split(rownames(hallmarks_circuits_anot), ": "), function(x) x[1]))) 
n_hallmark <- data.frame(hallmark = colnames(hallmarks_circuits_anot), total = apply(hallmarks_circuits_anot, 2, sum))

# add nº of circuit a pathway has in the RP map to normalized the plot
table_pathways <- add_column(hallmarks_circuits_anot, "pathway" = sapply(str_split(rownames(hallmarks_circuits_anot), ": "), function(x) x[1])) %>% pivot_longer(cols = -pathway) %>% 
  ...
  add_column(n_cir = n_cir$Freq[match(.$pathway, n_cir$Var1)]) %>% 
  add_column(total_hallmark = n_hallmark$total[match(.$name, n_hallmark$hallmark)])

# Percentage of the hallmark normalized per nº of circuit the pathway has
table_pathways$percentage_Inpathway <- (table_pathways$count/table_pathways$n_cir)*100 
# Percentage of the hallmark normalized by ttoal annotations fo that hallmark in the RP_map
table_pathways$percentage_perHallm <- (table_pathways$count/table_pathways$total_hallmark)*100 

# 40 pathways
length(unique(table_pathways$pathway))
...
# Presence of the hallmark in the pathway: Ex 10Apoptosis_pathwayA / 5 circ_pathwayA
table_pathways_pretty$percentage_Inpathway <- round(table_pathways_pretty$percentage_Inpathway, digits = 0) 
# Percentage of the hallmark from total annotations of that hallmark : Ex 10Apoptosis_pathwayA / 138_totalApopAnot  
table_pathways_pretty$percentage_perHallm <- round(table_pathways_pretty$percentage_perHallm , digits = 0) 
...
# check (6)
length(unique(pivot_shapRel_drugbank_functions_validated$KDT))
...
# Here we transform the DF into a matrix of counts of hallmark per KDT
...
relevant_KDTbyfunc <- rownames_to_column(hallmarks_circuits_anot, "circuit") %>% merge(., distinct(pivot_shapRel_drugbank_functions[, c("KDT", "circuit")])) %>% .[, -c(1)]  %>% pivot_longer(cols = -KDT) %>%
  ...
  pivot_wider(names_from = name , values_from = count ) %>% column_to_rownames("KDT")

relevant_KDTbyfunc[is.na(relevant_KDTbyfunc)] <- 0
# reorder according to hallmark circle
relevant_KDTbyfunc <- rbind( TOTAL = n_hallmark$total[match(colnames(relevant_KDTbyfunc),n_hallmark$hallmark)] ,relevant_KDTbyfunc) %>% 
  .[, c("Fatty.acid.and.lipid.metabolism", "Apoptosis", "Neuronal", "DNA.integrity" ,"Inflammatory.response", "Stress.response","Necrosis", "Development","Sensory.and.stimuli.transduction")]
...
drug_byfunc <- rownames_to_column(hallmarks_circuits_anot, "circuit") %>% merge(., distinct(pivot_shapRel_drugbank_functions[,c("circuit", "DRUG")])) %>% .[, -c(1)]  %>% pivot_longer(cols = -DRUG) %>%
  ...
  summarize(count = sum(value)) %>% .[-which(.$count == 0),] %>%
  pivot_wider(names_from = name , values_from = count ) %>% column_to_rownames("DRUG") 
...
# Delete the same hallmark interactions
# Here we transform the DF into a matrix of counts of hallmark per KDT
# reorder according to hallmark circle
ha_ha <- ha_ha[ which(apply(ha_ha[,c(1,2)], 1, function(x) ifelse(length(unique(x))>1, TRUE, FALSE))),]%>%
  pivot_wider(names_from = name , values_from = count ) %>% column_to_rownames("Module") %>%
  .[, c("Fatty acid and lipid metabolism", "Apoptosis", "Neuronal", "DNA integrity" ,"Inflammatory response", "Stress response","Necrosis", "Development","Sensory and stimuli transduction")]
