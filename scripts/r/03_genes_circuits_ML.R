...
paths_disease <- sapply(names(entrez.list), function(x) {
  cualpath_dis(as.character(entrezIDgenes_ORPHA$entrezID), x)
})
...
cir_disease_entrez <- sapply(cir_disease, function(x) {
  get_entrez_cir(x)
})
...
af_cir <- lapply(cir_disease_entrez, function(x) {
  which_cir(entrezIDgenes_ORPHA$entrezID, x)
})
af_cir <- stack(af_cir)

cir_af <- data.frame(
  Circuit = get_path_names(metaginfo, names = as.character(af_cir$ind[af_cir$values == T])),
...
indexall_entrezs <- sapply(genes_hpo_long$HPO, function(x) {
  all(hpo_codes %in% x)
})
...
df_allRDHPO <- data.frame(
  Gene_symbol = mapIds(org.Hs.eg.db, keys = entrezs_allRDHPO, column = "SYMBOL", keytype = "ENTREZID"),
  Entrez = entrezs_allRDHPO
)
...
index14_entrezs <- sapply(genes_hpo_long$HPO, function(x) {
  which_cir14(hpo_codes, x)
})
...
index12_entrezs <- sapply(genes_hpo_long$HPO, function(x) {
  which_cir12(hpo_codes, x)
})
...
index10_entrezs <- sapply(genes_hpo_long$HPO, function(x) {
  which_cir10(hpo_codes, x)
})
entrezs_10RDHPO <- as.character(genes_hpo_long$entrez_id[index10_entrezs])

sum(entrezs_10RDHPO %in% pathways$all.genes)

df_10RDHPO <- data.frame(
  Gene_symbol = mapIds(org.Hs.eg.db, keys = entrezs_10RDHPO, column = "SYMBOL", keytype = "ENTREZID"),
  Entrez = entrezs_10RDHPO
)
...
index8_entrezs <- sapply(genes_hpo_long$HPO, function(x) {
  which_cir8(hpo_codes, x)
})
entrezs_8RDHPO <- as.character(genes_hpo_long$entrez_id[index8_entrezs])

sum(entrezs_8RDHPO %in% pathways$all.genes)

df_8RDHPO <- data.frame(
  Gene_symbol = mapIds(org.Hs.eg.db, keys = entrezs_8RDHPO, column = "SYMBOL", keytype = "ENTREZID"),
  Entrez = entrezs_8RDHPO
)
...
for (i in 1:length(hpo_codes)) {
  ...
  index[[i]] <- sapply(genes_hpo_long$HPO, function(x) {
    which_cir_i(hpo_codes, x)
  })
  ...
  genes_sharingHPO$symbol_inHipathia[[i]] <- mapIds(
    org.Hs.eg.db,
    keys = genes_sharingHPO$genes_inHipathia[[i]],
    column = "SYMBOL",
    keytype = "ENTREZID"
  )
...
stacked_hpos <- genes_sharingHPO[, c("shared_hpos", "count", "not_in_ORPHA")] %>%
  dplyr::rename("Genes added to ORPHA/OMIM" = "not_in_ORPHA") %>%
  add_column("Total genes" = (genes_sharingHPO$count - genes_sharingHPO$not_in_ORPHA)) %>%
  mutate_all(as.character) %>%
  .[, c("shared_hpos", "Total genes", "Genes added to ORPHA/OMIM")] %>%
  pivot_longer(-shared_hpos)
...
paths_RD <- sapply(names(entrez.list), function(x) {
  cualpath_dis(as.character(RD_entrezs), x)
})

paths_RD <- names(entrez.list)[paths_RD]
RPgenes_inPaths <- sapply(names(entrez.list), function(x) {
  w <- intersect(RD_entrezs, entrez.list[[x]])
  if (length(w) != 0) {
...
RP_pathways_with_genes <- data.frame(RPgenes_inPaths) %>%
  add_column(Pathway = path_list$V1[match(.$ind, path_list$V2)], .before = "ind")
...
cir_RD <- subpathways.list[paths_RD] %>%
  unlist(.) %>%
  as.vector(.)
cir_RD_entrez <- sapply(cir_RD, function(x) {
  get_entrez_cir(x)
})
RD_cir <- lapply(cir_RD_entrez, function(x) {
  which_cir(RD_entrezs, x)
})
RD_cir <- stack(RD_cir)

cir_RD_names <- data.frame(
  Circuit = get_path_names(metaginfo, names = as.character(RD_cir$ind[RD_cir$values == T])),
  Hipathia_code = RD_cir$ind[RD_cir$values == T],
  stringsAsFactors = F
)

write.xlsx(cir_RD_names,
  file = file.path(tables_folder, paste0("cirHPO", disease, ".xlsx")),
  row.names = F
)
...
RP_map_annotations <- data.frame(
  circuit_code = circuits$hipathia[circuits$in_disease == T],
  ...
  GO_functions = sapply(circuits$hipathia[circuits$in_disease == T], function(x) {
    paste0(annotations_eff$Term[annotations_eff$pathway %in% x], collapse = ",")
  })
) %>%
  .[, -c(3)]

write.xlsx(RP_map_annotations, file = here("data", "interim", "RP_map_functions.xlsx"))
write.table(RP_map_annotations,
  file = here("data", "interim", "RP_map_functions.tsv"),
